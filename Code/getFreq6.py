...
def GetInputFromSent(conf,sent):
...
def GetOptionsFreq(fdist,opListPath):
	...
	for option in fullConfigOptions:
		if option in fdistList:
			optionDict[option] = fdist[option]
...
def GetWorkloadFromSent(conf,sent):
	sent = sent.lower()
	## Get throughput
	# Pattern to match: 197 requests/sec - 28.6 kB/second - 148 B/reques
	# Pattern to match: 100 files per second
	m = re.search(r'[\d|\.]+\s[a-z]+\s?(\/|per)\s?[a-z]+',sent,re.IGNORECASE)
	if m:
		print 'throughput:%s' % m.group(0)
...
def GetSympSupport(sympName,sent):
	#print 'sympName: %s' % sympName
	if sympName.lower() == 'cpu':
		# Get numbers
		#m = re.search('(\d+).*cpu|cpu.*(\d+)',sent)
		m = re.search('.*?%.*?.*cpu',sent,flags=re.IGNORECASE)
		if m != None:
			print m.group(0)	
			return True
		# *?: non greedy matching
	...
	if re.search(re.escape(sympName),str(sent),flags=re.IGNORECASE):
		result = cp.parse(sent)
		#print '%s' % result
		for subtree in result.subtrees():
			#print '%s' % str(subtree)
			if (subtree.label() == 'NP' and re.search(re.escape(sympName),str(subtree),flags=re.IGNORECASE)):
				print subtree
...
def GetSignature(dirRoot,inFileName,conf,ptnDict,metaPath):
	...
	for sent in sents:
		sent = sent.lower()
		foundSym, coreSymp = IsSympSent(conf,sent)
		if foundSym:
			print 'Core Symp: %s' % coreSymp
			UpdatePtnDictCount(ptnDict,'Symptom',coreSymp)
			# Get action
			foundAction, coreAction = GetActFromSent(conf,sent)
			if foundAction:
				# Update counter
				UpdatePtnDictCount(ptnDict,'Pattern','action')
				print 'Core Action: %s' % coreAction
				UpdatePtnDictCount(ptnDict,'Actions',coreAction)
				# Get input
				foundInput, coreInput = GetInputFromSent(conf,sent)
				if foundInput:
					UpdatePtnDictCount(ptnDict,'Pattern','input')
					print 'Core Input: %s' % coreInput
					UpdatePtnDictCount(ptnDict,'Input',coreInput)

		foundThput,coreThput = GetWorkloadFromSent(conf,sent)
		if foundThput:
			UpdatePtnDictCount(ptnDict,'Pattern','load')
			print 'Throughput: %s' % coreThput
			UpdatePtnDictCount(ptnDict,'Load',coreThput)
		#	break
	# end of for

	for k in ptnDict.keys():
		sympDict = ptnDict[k]
		sortedSympDict = sorted(sympDict.items(),key=operator.itemgetter(1),reverse=True)
		WriteToStdErr(k + ':' +str(sortedSympDict))
	...
	selectedOps,simScores = GetInferredOp(inFilePath,conf,50)
