		...
		with open(outFilePath, 'w') as fout:
			...
			for item in title_data:
				...
				fout.write(title + '\n')
						...
						# subheader_data = soup.find_all("p", {"class": "subheader"})
						# for item in subheader_data:
						#     item = str(item.text.encode("utf-8"))
				...
				g_data = soup.find_all("pre", {"class": "bz_comment_text"})
				for item in g_data:
				#sent = item.text
				#sent = item.text.encode("utf-8")
				...
				#sent = re.sub(r'b\"', '', sent)
					...
					# print(item.text.encode("utf-8"), file=fout)
	return
...
for op,val in ops:
	if op == '-o':
		...
		print 'Usage: python webCrawler.py -o ./WebCopora/ -f apacheBugIds'
		sys.exit(0)
	elif op == '-f':
		urlFile = val	
	else:
